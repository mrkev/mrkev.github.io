...
(function() {
  ...
  p.applyFilter = function(ctx) {
    ...
    hypotenuse =
      Math.ceil(hypotenuse / this.pixelsPerPoint) * this.pixelsPerPoint;
    ...
    for (var cIndex = 0; cIndex < this.colors.length; cIndex++) {
      ...
      var h = tsunami.geom.Point.polar(
        hypotenuse,
        (color.angle * Math.PI) / 180
      );
      var v = tsunami.geom.Point.polar(
        hypotenuse,
        ((color.angle + 90) * Math.PI) / 180
      );

      origin = new tsunami.geom.Point(
        width / 2 - h.x / 2 - v.x / 2,
        height / 2 - h.y / 2 - v.y / 2
      );
